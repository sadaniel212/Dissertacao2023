	...
	<ul v-if="items" role="menu">
		<template v-for="(item, i) of items">
			<li v-if="visible(item) && !item.separator" :key="item.label || i"
				...
				role="none">
				<div class="layout-menuitem-root-text" style="text-transform: uppercase" v-if="root">{{ item.label }}
				</div>
				<router-link v-if="item.to" :to="item.to" :style="item.style"
					:class="[item.class, 'p-ripple', { 'p-disabled': item.disabled }]" :target="item.target" exact
					active-class="active-route" @click="onMenuItemClick($event, item, i)"
					@mouseenter="onMenuItemMouseEnter(i)" role="menuitem" v-ripple>
				...
				<transition name="layout-menu">
					<appsubmenu
						...
						:items="visible(item) && item.items" :menuActive="menuActive" :layoutMode="layoutMode"
